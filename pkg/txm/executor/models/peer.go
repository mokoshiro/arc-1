...
var PeerWhere = struct {
	...
	ID:         whereHelperint{field: "`peer`.`id`"},
	PeerID:     whereHelperstring{field: "`peer`.`peer_id`"},
	Addr:       whereHelperstring{field: "`peer`.`addr`"},
	Credential: whereHelperstring{field: "`peer`.`credential`"},
	Location:   whereHelperstring{field: "`peer`.`location`"},
	CreatedAt:  whereHelpernull_Time{field: "`peer`.`created_at`"},
	UpdatedAt:  whereHelpertime_Time{field: "`peer`.`updated_at`"},
}
...
var (
	peerAllColumns            = []string{"id", "peer_id", "addr", "credential", "location", "created_at", "updated_at"}
	peerColumnsWithoutDefault = []string{"peer_id", "addr", "credential", "location"}
	peerColumnsWithDefault    = []string{"id", "created_at", "updated_at"}
...
func (o *Peer) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `peer` () VALUES ()%s%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for peer")
	}
...
func (o *Peer) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "peer", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `peer` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for peer")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for peer")
	}
