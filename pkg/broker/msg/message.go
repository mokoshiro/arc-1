...
type RoomNotificationRequest struct {
	targets []string `json:"targets"` // target peers id
}

type RoomNotificationResponse struct {
}

func (m *RoomNotificationRequest) Raw() []byte {
	b, err := json.Marshal(m)
	if err != nil {
		return nil
	}
...
func (m *RoomNotificationRequest) Unmarshal(b []byte) error {
	return json.Unmarshal(b, m)
}

func (m *RoomNotificationResponse) Raw() []byte {
	b, err := json.Marshal(m)
	if err != nil {
		return nil
	}
